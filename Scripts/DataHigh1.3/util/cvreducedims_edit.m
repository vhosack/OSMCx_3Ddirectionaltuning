function [projs mse like lat] = cvreducedims_edit(D, alg, dims, handles)  
...
%                         train_mask, test_mask, wbar, bar_time);
                    binWidth = 2;
                    wbar = []; bar_time = [];
                    [like_fold mse_fold p] = GPFACV(D, dims(idim), ifold, binWidth, ...
                        train_mask, test_mask, wbar, bar_time);
            end
...
function [like mse projs] = GPFACV(D, dim, fold, binWidth, train_mask, test_mask, wbar, bar_time)
   ...
   % [params gpfa_traj chugsLL] = gpfa_engineDH(train_data,dim, 'binWidth', binWidth, 'wbar', wbar, 'bar_time', bar_time);
    [params gpfa_traj chugsLL] = gpfa_engineDH(train_data,dim, 'binWidth', binWidth);
    ...
    cv_gpfa_struct = (cosmoother_gpfa_viaOrth_fast(test_data,params,dim));
    
    % get field names of struct, 7th name is cv pred, since 2 fields
    % removed? Was having issues with cell2struct...
    fnames = fieldnames(cv_gpfa_struct);
    
    cvdata = [cv_gpfa_struct.(fnames{7})]; % works for first dim only
    mse = sum(sum((cvdata-[test_data.data]).^2));
