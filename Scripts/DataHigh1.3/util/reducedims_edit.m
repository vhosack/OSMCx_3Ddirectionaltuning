function [newD, C, lat, explained, params] = reducedims_edit(D,alg, dims, handles)
...
% 
        ...
        s = getSeq(d, 10, 'useSqrt', false);%handles.binWidth, 'useSqrt', handles.use_sqrt);
...
%     if get(handles.kern_slider,'Value') ~= 0
        for i = 1:length(D)
            D(i).data = smoother(D(i).data, 10,10);%get(handles.kern_slider,'Value'), handles.binWidth);
        end
%     end
    ...
    switch alg
        ...
        case 2
            [newD C lat explained, params] = PPCAreduce(D,dims);
        case 3
            [newD C lat explained, params] = FAreduce(D,dims);
        case 4
            [newD C] = LDAreduce(D,dims);
        case 5
%             handles.binWidth = 10;
            [newD C lat explained params] = GPFAreduce(D,dims, handles.binWidth);
        otherwise
...
function [newD C lat explained, params] = PPCAreduce(D,dims)
...
%   considered together to get the best joint reduction.
    ...
    newD = D;
    C = loadings;
    [chugs, lat, explained] = pcacov(params.L * params.L');
    lat = cumsum(lat(1:dims))./sum(lat(1:dims));
end

function [newD C lat explained, params] = FAreduce(D,dims)
...
function [newD, C, lat, explained, params] = GPFAreduce(D,dims, binWidth)
...
%   of GPFAENGINE, by Byron Yu and John Cunningham.
    ...
    delete(wb);
    C = params.Corth;
    [chugs, lat, explained] = pcacov(params.C * params.C');
    lat = cumsum(lat(1:dims))./sum(lat(1:dims));
